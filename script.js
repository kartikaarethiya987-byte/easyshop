...
function renderOrdersPage() {
  ...
  container.innerHTML = orders.map(order => {
    ...
    const deliveryInfo   = [
      order.companyName  ? `<div class="order-del-chip"><i class="fa-solid fa-building"></i> ${order.companyName}</div>` : "",
      order.address      ? `<div class="order-del-chip"><i class="fa-solid fa-location-dot"></i> ${order.address}</div>` : "",
...
/* ================================================================
   SECTION 18 – ADVANCED CHECKOUT FORM & DELIVERY SYSTEM
   ──────────────────────────────────────────────────────────────
   showCheckoutForm()        — renders the full checkout overlay
   generateDeliverySlots()   — builds dropdown for next 3 days
   calculateExtraCharge()    — <24 hrs = ₹100, else 0
   initGoogleMapsAutocomplete() — wires Places API on address field
   handleDeliveryTimeChange()   — live charge preview
   confirmOrder()            — validates, builds order, saves, redirects
   ================================================================ */

/* ── Module-level state for the checkout form ─────────────────── */
let _checkoutAddress    = "";   // formatted address from Google Maps
let _checkoutLat        = null; // latitude (optional)
let _checkoutLng        = null; // longitude (optional)
let _checkoutCharge     = 0;    // delivery extra charge (0 or 100)
let _checkoutPayMethod  = "";   // chosen payment method

/* ──────────────────────────────────────────────────────────────
   showCheckoutForm()
   Creates a full-screen overlay with the checkout form.
   Called by proceedToCheckout() after confirming login.
   ────────────────────────────────────────────────────────────── */
function showCheckoutForm() {
  ...
  /* Pre-compute prices so the form can show the running total */
  const subtotal   = getCartTotal();
  ...
  /* Min time = right now (can't pick the past) */
  const now = new Date();
  const minTimeStr =
  ...
  overlay.setAttribute("role", "dialog");
  overlay.setAttribute("aria-modal", "true");
  overlay.setAttribute("aria-label", "Checkout form");
  ...
  overlay.innerHTML = `
    <div class="co-box" id="coBox">
      ...
      <div class="co-header">
        ...
        <button class="co-close" id="coCloseBtn" aria-label="Close checkout">
          <i class="fa-solid fa-xmark"></i>
        </button>
      </div>
      ...
      <!-- Step breadcrumb -->
      <div class="co-steps">
        <span class="co-step co-step--active">
          <i class="fa-solid fa-location-dot"></i> Delivery
        </span>
      ...
      <div class="co-body">
        ...
        <!-- ── Company Name ───────────────────────────────────── -->
        <div class="form-group">
          <label class="form-label co-label" for="coCompany">
            <i class="fa-solid fa-building"></i> Company Name <span class="req">*</span>
          </label>
          <input type="text" id="coCompany" class="form-input"
            placeholder="Your company or personal name"/>
          <span class="field-error" id="coCompanyErr"></span>
        </div>

        <!-- ── Delivery Address (Google Maps autocomplete) ────── -->
        <div class="form-group">
          <label class="form-label co-label" for="coAddress">
            <i class="fa-solid fa-location-dot"></i> Delivery Address <span class="req">*</span>
          </label>
          <div class="co-address-wrap">
            <input type="text" id="coAddress" class="form-input"
              placeholder="Start typing your address…"
              autocomplete="off"/>
            <button type="button" class="co-map-btn" onclick="useCurrentLocation()"
          ...
          <span class="field-error" id="coAddressErr"></span>
        </div>

        <!-- ── Delivery Date & Time ──────────────────────────── -->
        <div class="form-group">
          <label class="form-label co-label" for="coDeliveryTime">
            <i class="fa-regular fa-calendar-check"></i> Preferred Delivery Date &amp; Time
            <span class="req">*</span>
          </label>
          ...
          <!-- Live charge preview updates as user picks time -->
        ...
        <div class="form-group">
          ...
          <select id="coSlot" class="form-input">
            ${slotOptions}
          </select>
        </div>
          ...
          <label class="co-fast-label" for="coFastDelivery">
            ...
            <span class="co-fast-inner">
              ...
              <span>
                ...
                <small>Guaranteed delivery within 12 hours</small>
              </span>
              <span class="co-fast-price">+₹100</span>
            </span>
          </label>
          <!-- Dynamic message shown/hidden by JS -->
          <div class="co-fast-msg" id="coFastMsg" style="display:none;">
            <i class="fa-solid fa-bolt"></i>
            Fast delivery surcharge of <strong>₹100</strong> will be added to your total.
          </div>
        </div>
        ...
        <div class="co-total-preview">
          ...
          <div class="co-total-row" id="coDeliveryRow">
        ...
        <div class="form-group">
          <label class="form-label co-label">
            <i class="fa-solid fa-credit-card"></i> Payment Method <span class="req">*</span>
          </label>
          <div class="co-pay-methods">
            <label class="co-pay-opt">
              <input type="radio" name="coPayMethod" value="UPI" id="pmUpi"/>
              <span class="co-pay-inner">
                <i class="fa-solid fa-mobile-screen-button"></i> UPI
              </span>
            </label>
            <label class="co-pay-opt">
              <input type="radio" name="coPayMethod" value="Card" id="pmCard"/>
              <span class="co-pay-inner">
                <i class="fa-solid fa-credit-card"></i> Card
              </span>
            </label>
            <label class="co-pay-opt">
              <input type="radio" name="coPayMethod" value="Cash on Delivery" id="pmCod"/>
              <span class="co-pay-inner">
  ...
  /* Wire delivery-time input for live charge preview */
  const timeInput = document.getElementById("coDeliveryTime");
  timeInput.addEventListener("change", handleDeliveryTimeChange);
  handleDeliveryTimeChange(); // run once to set initial state

  /* Wire fast-delivery checkbox */
  ...
  /* Initialise Google Maps autocomplete if the API is loaded */
  if (window.google && window.google.maps && window.google.maps.places) {
    initGoogleMapsAutocomplete();
  } else {
    /* API not loaded (key not set) — graceful fallback: plain text input */
    const hint = document.querySelector(".co-hint");
    if (hint) hint.innerHTML =
      '<i class="fa-solid fa-circle-info"></i> Enter your full delivery address.';
  }
}
...
/* ──────────────────────────────────────────────────────────────
   ...
   Builds an array of slot strings for the next 3 calendar days.
   Each day gets 4 slots: 9–12, 12–3, 3–6, 6–9.
   ────────────────────────────────────────────────────────────── */
function generateDeliverySlots() {
  ...
  for (let d = 0; d < 3; d++) {
    const date = new Date();
    date.setDate(date.getDate() + d + 1); // tomorrow, day after, two days after
    const label = d === 0 ? "Tomorrow"
      : `${dayNames[date.getDay()]} ${date.getDate()} ${months[date.getMonth()]}`;
    slotTimes.forEach(t => slots.push(`${label} · ${t}`));
  }
  return slots;
}

/* ──────────────────────────────────────────────────────────────
   calculateExtraCharge(selectedTimeStr)
   Returns 100 if the selected time is < 24 hours from now.
   Returns 0 otherwise.
   ────────────────────────────────────────────────────────────── */
function calculateExtraCharge(selectedTimeStr) {
  if (!selectedTimeStr) return 0;
  const selected = new Date(selectedTimeStr).getTime();
  const now      = Date.now();
  const diffMs   = selected - now;
  return diffMs > 0 && diffMs < 24 * 60 * 60 * 1000 ? 100 : 0;
}

/* ──────────────────────────────────────────────────────────────
   handleDeliveryTimeChange()
   Called whenever the datetime input changes.
   Updates the charge badge and recalculates the total.
   ────────────────────────────────────────────────────────────── */
function handleDeliveryTimeChange() {
  const timeVal   = document.getElementById("coDeliveryTime")?.value || "";
  const charge    = calculateExtraCharge(timeVal);
  _checkoutCharge = charge;

  const badge    = document.getElementById("coChargeBadge");
  const chargeEl = document.getElementById("coChargeText");

  if (charge > 0) {
    /* < 24 hours: fast-delivery surcharge applies */
    if (badge)    { badge.classList.add("co-charge-badge--fast"); }
    if (chargeEl) {
      chargeEl.innerHTML =
        `<i class="fa-solid fa-bolt"></i>
         Fast delivery surcharge: <strong>₹100</strong>
         (selected time is within 24 hours)`;
    }
  } else {
    if (badge)    { badge.classList.remove("co-charge-badge--fast"); }
    if (chargeEl) { chargeEl.textContent = "Standard delivery – FREE"; }
  }
...
/* ──────────────────────────────────────────────────────────────
   recalcTotal()
   Recalculates the grand total shown in the checkout form,
   combining subtotal, 5% discount, fast-delivery charge, and
   the fast-delivery checkbox.
   ────────────────────────────────────────────────────────────── */
function recalcTotal() {
  ...
  /* Charge = auto-time-charge OR manual fast-delivery checkbox */
  const finalCharge  = _checkoutCharge > 0 ? _checkoutCharge : (fastChecked ? 100 : 0);
  const grandTotal   = subtotal - discount + finalCharge;

  /* Update DOM */
  const amtEl  = document.getElementById("coDeliveryAmt");
  const totEl  = document.getElementById("coGrandTotal");
  if (amtEl) {
    amtEl.textContent = finalCharge > 0 ? formatPrice(finalCharge) : "FREE";
    amtEl.className   = finalCharge > 0 ? "fast-charge-tag" : "free-tag";
  }
...
/* ──────────────────────────────────────────────────────────────
   initGoogleMapsAutocomplete()
   Attaches Google Places Autocomplete to the address input.
   Stores formatted_address and lat/lng when the user picks
   a suggestion from the dropdown.

   Requires: Google Maps JS API with places library
   Script tag (already in cart.html):
...
function initGoogleMapsAutocomplete() {
  ...
  const autocomplete = new google.maps.places.Autocomplete(input, {
    types:  ["geocode", "establishment"],   // addresses + business names
  ...
  autocomplete.addListener("place_changed", function () {
    ...
    if (place.formatted_address) {
      _checkoutAddress = place.formatted_address;
      input.value      = place.formatted_address;
    } else if (place.name) {
      _checkoutAddress = place.name;
    }

    if (place.geometry?.location) {
      _checkoutLat = place.geometry.location.lat();
      _checkoutLng = place.geometry.location.lng();
    }

    /* Clear any existing address error */
    const errEl = document.getElementById("coAddressErr");
    if (errEl) errEl.textContent = "";
  });
}
...
/* ──────────────────────────────────────────────────────────────
   useCurrentLocation()
   Attempts to fill the address field with the user's GPS location
   using the browser Geolocation API + Google Geocoding.
   ────────────────────────────────────────────────────────────── */
function useCurrentLocation() {
  ...
  const input = document.getElementById("coAddress");
  if (input) input.placeholder = "Detecting your location…";
  ...
  navigator.geolocation.getCurrentPosition(
    function (pos) {
      ...
      /* Reverse geocode if Google Maps API is loaded */
      if (window.google?.maps) {
        const geocoder = new google.maps.Geocoder();
        geocoder.geocode({ location: { lat, lng } }, function (results, status) {
          if (status === "OK" && results[0]) {
            _checkoutAddress = results[0].formatted_address;
            if (input) input.value = _checkoutAddress;
          } else {
            if (input) input.value = `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
            _checkoutAddress = input.value;
          }
        });
      } else {
        /* No Maps API – just use raw coordinates */
        if (input) input.value = `${lat.toFixed(6)}, ${lng.toFixed(6)}`;
        _checkoutAddress = input.value;
      }
    },
...
/* ──────────────────────────────────────────────────────────────
   ...
   2. Calculate final delivery charge
   3. Confirm with the user (window.confirm)
   4. Build the order object (new expanded structure)
   5. Reduce stock per item
   6. Save to user.orders, clear cart
   7. Show success animation then redirect to orders.html
   ────────────────────────────────────────────────────────────── */
function confirmOrder() {
  ...
  const addressInput = (document.getElementById("coAddress")?.value  || "").trim();
  ...
  /* Use Google Maps-resolved address if available, else raw input */
  const finalAddress = _checkoutAddress || addressInput;
  ...
  clrE("coCompanyErr"); clrE("coAddressErr");
  clrE("coTimeErr");    clrE("coPayErr");

  if (!companyName)    setE("coCompanyErr", "Please enter a company / recipient name.");
  if (!finalAddress)   setE("coAddressErr", "Please enter a delivery address.");
  if (!deliveryTime)   setE("coTimeErr",    "Please select a delivery date and time.");
  if (!payMethod)      setE("coPayErr",     "Please select a payment method.");
  if (!valid) return;
    ...
    if (!p || item.quantity > p.stock) {
      showCheckoutMessage("error",
        `<i class="fa-solid fa-circle-xmark"></i>
         "${item.name}" stock changed.  Please <a href="cart.html">review your cart</a>.`
      );
      return;
    }
  }
  ...
  /* ── Build order object (expanded structure) ───────────────── */
  const order = {
    orderId:        "ORD-" + Date.now(),
    date:           new Date().toISOString(),
    items:          [...cart],
  ...
  /* ── Persist: reduce stock, save order, clear cart ────────── */
  cart.forEach(item => reduceStock(item.productId, item.quantity));
  user.orders.unshift(order);
  user.cart = [];
  saveOneUser(user);
  ...
  document.getElementById("coStepPay")?.classList.add("co-step--active");
  document.getElementById("coStepDone")?.classList.add("co-step--active");

  /* ── Success animation inside the overlay ──────────────────── */
  ...
  renderCartPage(); // refresh cart below the overlay
}

/* ──────────────────────────────────────────────────────────────
   showCheckoutMessage(type, html)
   Shows a coloured message inside the checkout overlay.
   ────────────────────────────────────────────────────────────── */
