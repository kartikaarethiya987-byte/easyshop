...
function processCheckout(paymentMethod) {
  ...
  const order = {
    ...
    status:        "Paid"   // Always "Paid" after going through payment flow
  };
...
function renderCartPage() {
  ...
  /*
   * Phase 8: "Proceed to Payment" opens the 3-step payment modal
   * (openPaymentModal lives in script.js §21).
   * If the user is not logged in, show a login CTA instead.
   */
  const checkoutBlock = isLoggedIn()
    ? `<button class="checkout-btn checkout-btn--pay" id="checkoutBtn"
               onclick="openPaymentModal()">
         <i class="fa-solid fa-bolt"></i> Proceed to Payment
       </button>`
    : `<a href="login.html" class="checkout-btn checkout-btn--login">
         <i class="fa-solid fa-right-to-bracket"></i> Login to Checkout
       </a>`;

  /* Use the Phase 8 helper that includes payment method icons */
  layout.innerHTML = `
    <div class="cart-items-list">${itemsHTML}</div>
    ${renderCartSummary(subtotal, delivery, discount, grandTotal)}
  `;
  /* "Proceed to Payment" button uses onclick="openPaymentModal()" via renderCartSummary */
}
...
/* ============================================================
   SECTION 21 – PAYMENT SIMULATION SYSTEM  (Phase 8 — NEW)

   How it works (4 steps, all in a modal overlay):
   ─────────────────────────────────────────────────────────────
   Step 1 – Method selection  : UPI | Card | Cash on Delivery
   Step 2 – Payment form      : UPI ID  OR  card fields
   Step 3 – Confirmation      : native confirm() dialog
   Step 4 – Success screen    : 🎉 order saved, cart cleared

   Entry point:  openPaymentModal()   ← called from "Proceed to
                                         Payment" button in cart
   ============================================================ */

/* ── Internal state for the payment flow ─────────────────────── */
let _payMethod  = "";   // "upi" | "card" | "cod"
let _payTotal   = 0;    // grand total at the time modal opens


/* ──────────────────────────────────────────────────────────────
   openPaymentModal()
   Creates the modal DOM, shows Step 1 (method selection).
   Called by the "Proceed to Payment" button rendered in
   renderCartPage() → renderCartSummary().
   ────────────────────────────────────────────────────────────── */
function openPaymentModal() {
  ...
  /* Calculate the same grand total shown in the order summary */
  const subtotal   = getCartTotal();
  const delivery   = subtotal > 500 ? 0 : 49;
  const discount   = Math.round(subtotal * 0.05);
  _payTotal        = subtotal + delivery - discount;

  /* Remove any leftover modal from a previous open */
  ...
  overlay.setAttribute("aria-labelledby", "payModalTitle");

  overlay.innerHTML = `
    <div class="pay-box" id="payBox">
      <!-- Header -->
      <div class="pay-header">
        <h2 class="pay-title" id="payModalTitle">
          <i class="fa-solid fa-lock pay-lock-icon"></i>
          Secure Payment
        </h2>
        <button class="pay-close" id="payCloseBtn" aria-label="Close payment modal">
      ...
      <!-- Dynamic content area — steps are swapped in here -->
...
/* ──────────────────────────────────────────────────────────────
   closePaymentModal()
   Removes the modal and restores body scroll.
   ────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   activateStep(n)
   Updates the step indicator dots to highlight step n.
   ────────────────────────────────────────────────────────────── */
...
/* ──────────────────────────────────────────────────────────────
   ...
   Shows three clickable method cards.
   ────────────────────────────────────────────────────────────── */
function showPayStep1() {
  ...
  body.innerHTML = `
    ...
    <div class="pay-methods">
      ...
      <!-- UPI option -->
      <button class="pay-method-card" id="optUpi" onclick="selectPayMethod('upi')">
      ...
      <button class="pay-method-card" id="optCard" onclick="selectPayMethod('card')">
      ...
      <button class="pay-method-card" id="optCod" onclick="selectPayMethod('cod')">
    ...
    <!-- UPI partner logos strip -->
    <div class="pay-logos">
      <span class="pay-logo-chip">GPay</span>
      <span class="pay-logo-chip">PhonePe</span>
      <span class="pay-logo-chip">Paytm</span>
      <span class="pay-logo-chip">BHIM</span>
      <span class="pay-logo-chip">Amazon Pay</span>
    </div>
  `;
}
...
/* ──────────────────────────────────────────────────────────────
   ...
   Highlights the chosen card and routes to the correct Step 2.
   ────────────────────────────────────────────────────────────── */
function selectPayMethod(method) {
  _payMethod = method;

  /* Highlight selected card briefly before advancing */
  const ids = { upi: "optUpi", card: "optCard", cod: "optCod" };
  const el  = document.getElementById(ids[method]);
  ...
  body.innerHTML = `
    ...
    <div class="pay-form">
      <div class="form-group">
        <label class="form-label" for="upiId">
          UPI ID <span class="req">*</span>
        </label>
      ...
      <!-- UPI app logos -->
      <div class="upi-apps">
        <span class="upi-app-chip" onclick="document.getElementById('upiId').value='yourname@gpay'">
          <i class="fa-brands fa-google-pay"></i> GPay
        </span>
        <span class="upi-app-chip" onclick="document.getElementById('upiId').value='yourname@ybl'">
          <i class="fa-solid fa-phone"></i> PhonePe
        </span>
        <span class="upi-app-chip" onclick="document.getElementById('upiId').value='yourname@paytm'">
          <i class="fa-solid fa-p"></i> Paytm
        </span>
      </div>
...
function showPayStep2Card() {
  ...
  body.innerHTML = `
    ...
    <div class="pay-form">
      ...
      <div class="form-group">
        <label class="form-label" for="cardName">Name on Card <span class="req">*</span></label>
  ...
  /* Live card name preview */
  document.getElementById("cardName").addEventListener("input", function () {
    const el = document.getElementById("cardNameDisplay");
    el.textContent = this.value.toUpperCase() || "YOUR NAME";
  });
  ...
  body.innerHTML = `
    ...
    <div class="cod-info-box">
      <div class="cod-info-row">
        ...
        <span>Keep <strong>${formatPrice(_payTotal)}</strong> ready at the time of delivery.</span>
      </div>
      <div class="cod-info-row">
        <i class="fa-solid fa-circle-check cod-check"></i>
        <span>Expected delivery in <strong>3–5 business days</strong>.</span>
      </div>
      <div class="cod-info-row">
        <i class="fa-solid fa-circle-check cod-check"></i>
...
/* ──────────────────────────────────────────────────────────────
   ...
   STEP 3 — Validates form fields then shows native confirm().
   On confirmation calls finalisePayment().
   ────────────────────────────────────────────────────────────── */
function validateAndConfirmPayment() {
  ...
  if (_payMethod === "upi") {
    ...
    if (!upiId) {
      if (errEl) errEl.textContent = "Please enter your UPI ID.";
      document.getElementById("upiId")?.focus();
      return;
    }
    /* Basic UPI format: something@something */
    if (!/^[a-zA-Z0-9._+-]+@[a-zA-Z]+$/.test(upiId)) {
      if (errEl) errEl.textContent = "Invalid UPI ID format (e.g. name@upi).";
      document.getElementById("upiId")?.focus();
      return;
    }
  ...
  if (_payMethod === "card") {
    ...
    if (cardNum.length < 16) {
      const el = document.getElementById("cardNumError");
      if (el) el.textContent = "Enter a valid 16-digit card number.";
      cardValid = false;
    } else {
      const el = document.getElementById("cardNumError");
      if (el) el.textContent = "";
    ...
    if (!cardName) {
      ...
      if (el) el.textContent = "Enter the name on your card.";
      cardValid = false;
    } else {
      const el = document.getElementById("cardNameError");
      if (el) el.textContent = "";
    ...
    if (!/^\d{2}\/\d{2}$/.test(cardExp)) {
      const el = document.getElementById("cardExpError");
      if (el) el.textContent = "Enter expiry as MM/YY.";
      cardValid = false;
    } else {
      const el = document.getElementById("cardExpError");
      if (el) el.textContent = "";
    ...
    if (cardCvv.length < 3) {
      const el = document.getElementById("cardCvvError");
      if (el) el.textContent = "Enter a valid CVV (3–4 digits).";
      cardValid = false;
    } else {
      const el = document.getElementById("cardCvvError");
      if (el) el.textContent = "";
  ...
  /* COD has no form fields to validate */

  /* ── Step 3: native confirm dialog ──────────────────────── */
  const methodLabels = { upi: "UPI", card: "Credit/Debit Card", cod: "Cash on Delivery" };
  const label        = methodLabels[_payMethod] || _payMethod;

  const confirmed = window.confirm(
    `Confirm payment of ${formatPrice(_payTotal)} via ${label}?\n\n` +
...
/* ──────────────────────────────────────────────────────────────
   finalisePayment()
   STEP 4 — Creates the order, reduces stock, clears cart,
            shows the success screen inside the modal.
   ────────────────────────────────────────────────────────────── */
function finalisePayment() {
  ...
  /* ── Stock re-validation just before placing order ─────── */
  for (const item of cart) {
    const p = getProductById(item.productId);
    if (!p || item.quantity > p.stock) {
      closePaymentModal();
  ...
  /* ── Build the order object with payment info ─────────── */
  const subtotal   = getCartTotal();
  const delivery   = subtotal > 500 ? 0 : 49;
  const discount   = Math.round(subtotal * 0.05);
  const grandTotal = subtotal + delivery - discount;
  ...
  const order = {
    orderId:       "ORD-" + Date.now(),
    date:          new Date().toISOString(),
    items:         [...cart],          // snapshot of cart at purchase time
    subtotal:      subtotal,
    delivery:      delivery,
    discount:      discount,
    total:         grandTotal,
    totalAmount:   grandTotal,         // alias for compatibility
    paymentMethod: methodLabels[_payMethod] || _payMethod,
    status:        "Paid"              // New: "Paid" instead of "Delivered"
  };

  /* ── Reduce stock for every purchased item ──────────────── */
  cart.forEach(item => reduceStock(item.productId, item.quantity));

  /* ── Save order + clear cart on the user object ────────── */
  user.orders.unshift(order);
  user.cart = [];
  saveOneUser(user);
  updateCartBadge();
  ...
  /* Hide the close × button so user must choose a CTA */
  const closeBtn = document.getElementById("payCloseBtn");
  if (closeBtn) closeBtn.style.display = "none";

  /* Refresh the cart page underneath (shows empty cart) */
  renderCartPage();
}
...
/* ──────────────────────────────────────────────────────────────
   renderCartSummary(subtotal, delivery, discount, grandTotal)
   Helper that builds the right-hand Order Summary panel,
   replacing the old "Proceed to Checkout" button with the
   new "Proceed to Payment" button that opens the modal.
   ────────────────────────────────────────────────────────────── */
function renderCartSummary(subtotal, delivery, discount, grandTotal) {
  const payBlock = isLoggedIn()
    ? `<button class="pay-trigger-btn" id="payNowBtn" onclick="openPaymentModal()">
  ...
  return `
    <div class="order-summary">
      ...
      <p class="secure-note">
        ...
        Secure checkout &middot; 256-bit SSL
      </p>
      ...
      <div class="pay-accepted">
        ...
        <div class="pay-accepted-icons">
          <span title="UPI"><i class="fa-solid fa-mobile-screen-button"></i> UPI</span>
          <span title="Card"><i class="fa-solid fa-credit-card"></i> Card</span>
          <span title="COD"><i class="fa-solid fa-truck-fast"></i> COD</span>
        </div>
      </div>
    </div>
  `;
