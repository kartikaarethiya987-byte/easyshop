...
const KEYS = {
  ...
  ORDERS:   "se_orders",    // global array of ALL orders (for admin view)
};
...
function confirmOrder() {
  ...
  /* ── Also write to global orders list (admin can see all) ── */
...
function _opShowSuccessScreen(order) {
  ...
  page.innerHTML = `
    <div class="op-success-page">
      ...
      <!-- WhatsApp share button – sends order summary to business number -->
      <a class="op-btn op-btn--whatsapp"
         href="${_aoGetWhatsAppURL(order)}"
...
/* ================================================================
   SECTION 20 – GLOBAL ORDER STORE & ADMIN ORDERS VIEW
   ──────────────────────────────────────────────────────────────
   Orders are stored in TWO places:
     1. user.orders[]       – per-user history (existing)
     2. localStorage["se_orders"] – global flat list (NEW)
                              → readable by admin without looping
                                through all users

   PUBLIC FUNCTIONS
   ──────────────────────────────────────────────────────────────
   placeOrder(order, user)    — append to global se_orders list
   loadOrders()               — return all global orders (array)
   clearOrders()              — delete all from se_orders
   renderAdminOrders()        — render the "All Orders" section
   _aoOrderCard(order, idx)   — build one order card HTML string
   sendWhatsAppNotification() — open WhatsApp with order summary
   ================================================================ */

/* ──────────────────────────────────────────────────────────────
   placeOrder(order, user)
   ──────────────────────────────────────────────────────────────
   Called by submitOrder() and confirmOrder() after every
   successful purchase.  Appends an enriched order record to
   the global "se_orders" array in localStorage.

   The record adds:
     • userEmail  – so admin can see who placed it
     • userName   – display name for the admin table
   ────────────────────────────────────────────────────────────── */
function placeOrder(order, user) {
  /* Read the existing global list (never overwrite) */
  const raw    = localStorage.getItem(KEYS.ORDERS);
  const orders = raw ? JSON.parse(raw) : [];

  /* Enrich with user identity for the admin panel */
...
/* ──────────────────────────────────────────────────────────────
   loadOrders()
   ──────────────────────────────────────────────────────────────
   Returns the full global orders array, newest first.
   Admin panel calls this on page load.
   ────────────────────────────────────────────────────────────── */
function loadOrders() {
  const raw = localStorage.getItem(KEYS.ORDERS);
  return raw ? JSON.parse(raw) : [];
...
/* ──────────────────────────────────────────────────────────────
   clearOrders()
   ──────────────────────────────────────────────────────────────
   Removes ALL orders from the global store.
   Called by the "Clear All Orders" button in admin.html.
   ────────────────────────────────────────────────────────────── */
function clearOrders() {
  if (!confirm("Are you sure you want to permanently delete ALL orders?\nThis cannot be undone.")) {
    return;
  }
  localStorage.removeItem(KEYS.ORDERS);
  renderAdminOrders();            // re-render to show empty state
  renderAdminStats();             // update the stats cards
...
/* ──────────────────────────────────────────────────────────────
   renderAdminOrders()
   ──────────────────────────────────────────────────────────────
   Injects the "All Orders" section into #adminOrdersContainer.
   Called on admin.html page load and after clearOrders().
   ────────────────────────────────────────────────────────────── */
function renderAdminOrders() {
  ...
  if (!orders.length) {
    container.innerHTML = `
      <div class="ao-empty">
        ...
        <p class="ao-empty-sub">
          Orders placed through the store will appear here automatically.
        </p>
      </div>`;
    return;
  }
  ...
  /* ── Build order cards ───────────────────────────────────── */
  container.innerHTML = orders.map((order, idx) => _aoOrderCard(order, idx)).join("");
}


/* ──────────────────────────────────────────────────────────────
   _aoOrderCard(order, idx)
   ──────────────────────────────────────────────────────────────
   Returns the HTML string for one admin order card.
   Displays: Order ID, customer info, product list,
             totals, payment, delivery, WhatsApp button.
   ────────────────────────────────────────────────────────────── */
function _aoOrderCard(order, idx) {
  /* ── Customer info ───────────────────────────────────────── */
  const addr    = order.selectedAddress || {};
  const company = addr.companyName || order.companyName || "—";
  const name    = addr.fullName    || order.fullName    || order.userName || "—";
  const address = addr.address     || order.address     || "—";
  const phone   = addr.phone       || order.phone       || "—";

  /* ── Product rows ────────────────────────────────────────── */
  const items = (order.items || []).map(item => `
    <div class="ao-product-row">
      <img class="ao-product-img" src="${item.image}"
           alt="${item.name}"
           onerror="this.src='https://via.placeholder.com/44'"/>
      <div class="ao-product-info">
        <span class="ao-product-name">${item.name}</span>
        <span class="ao-product-meta">
  ...
  const statusClass = status.toLowerCase() === "paid" ? "ao-badge--paid" : "ao-badge--pending";

  /* ── WhatsApp URL ────────────────────────────────────────── */
  const waNumber = "91XXXXXXXXXX";   // ← Replace with your WhatsApp number
  const waText   = _aoWhatsAppText(order, name, company, address, phone);
  const waURL    = `https://wa.me/${waNumber}?text=${encodeURIComponent(waText)}`;

  /* ── Delivery charge display ─────────────────────────────── */
  const dcDisplay = (order.deliveryCharge || 0) > 0
    ? formatPrice(order.deliveryCharge) + " (Fast)"
    : "FREE";

  return `
    <div class="ao-card" id="aoCard-${order.orderId}">

      <!-- Card header: Order ID + date + status + WhatsApp -->
      <div class="ao-card-header">
        <div class="ao-card-meta">
          <span class="ao-order-id">
            <i class="fa-solid fa-receipt"></i> ${order.orderId}
          </span>
          <span class="ao-order-date">
        ...
        <div class="ao-card-actions">
          <span class="ao-badge ${statusClass}">
            <i class="fa-solid fa-circle-check"></i> ${status}
          </span>
          <!-- WhatsApp notification button -->
          <a class="ao-wa-btn" href="${waURL}" target="_blank" rel="noopener"
      ...
      <div class="ao-customer-grid">
        <div class="ao-customer-field">
          <i class="fa-solid fa-building"></i>
          <div>
            <span class="ao-field-label">Company</span>
            <span class="ao-field-val">${company}</span>
          </div>
        </div>
        <div class="ao-customer-field">
          <i class="fa-solid fa-user"></i>
          <div>
            <span class="ao-field-label">Customer</span>
            <span class="ao-field-val">${name}</span>
          </div>
        </div>
        <div class="ao-customer-field">
          <i class="fa-solid fa-phone"></i>
          <div>
            <span class="ao-field-label">Phone</span>
            <span class="ao-field-val">${phone}</span>
          </div>
        </div>
        <div class="ao-customer-field">
          <i class="fa-solid fa-envelope"></i>
          <div>
            <span class="ao-field-label">Email</span>
            <span class="ao-field-val">${order.userEmail || "—"}</span>
          </div>
        </div>
        <div class="ao-customer-field ao-customer-field--full">
          <i class="fa-solid fa-location-dot"></i>
          <div>
            <span class="ao-field-label">Delivery Address</span>
            <span class="ao-field-val">${address}</span>
          </div>
        </div>
        <div class="ao-customer-field">
          <i class="fa-solid fa-clock"></i>
          <div>
            <span class="ao-field-label">Slot</span>
            <span class="ao-field-val">${order.deliverySlot || "—"}</span>
          </div>
        </div>
        <div class="ao-customer-field">
          <i class="fa-solid fa-bolt"></i>
          <div>
            <span class="ao-field-label">Delivery Charge</span>
            <span class="ao-field-val">${dcDisplay}</span>
          </div>
        </div>
        <div class="ao-customer-field">
          <i class="fa-solid fa-wallet"></i>
          <div>
            <span class="ao-field-label">Payment</span>
            <span class="ao-field-val">${order.paymentMethod || "—"}</span>
          </div>
        </div>
      </div>
      ...
      <div class="ao-products">
        <p class="ao-products-label">
          <i class="fa-solid fa-box"></i>
          Items (${(order.items || []).length})
        </p>
        <div class="ao-products-list">${items}</div>
      </div>

      <!-- Price summary row -->
      <div class="ao-price-summary">
        <span class="ao-price-item">
          Subtotal: <strong>${formatPrice(order.subtotal || 0)}</strong>
        </span>
        <span class="ao-price-item ao-price-discount">
          Discount: <strong>−${formatPrice(order.discount || 0)}</strong>
        </span>
        <span class="ao-price-item">
          Delivery: <strong>${dcDisplay}</strong>
        </span>
        <span class="ao-price-total">
...
/* ──────────────────────────────────────────────────────────────
   _aoWhatsAppText(order, name, company, address, phone)
   ──────────────────────────────────────────────────────────────
...
function _aoWhatsAppText(order, name, company, address, phone) {
  ...
  (order.items || []).forEach(item => {
    lines.push(`  • ${item.name}  ×${item.quantity}  @ ₹${item.price}  = ₹${item.price * item.quantity}`);
  });

  lines.push("");
  lines.push("💰 *Price Breakdown*");
  lines.push(`  Subtotal:  ₹${order.subtotal || 0}`);
  lines.push(`  Discount:  −₹${order.discount || 0}`);
  lines.push(`  Delivery:  ${(order.deliveryCharge || 0) > 0 ? "₹" + order.deliveryCharge + " (Fast)" : "FREE"}`);
  lines.push(`  *TOTAL:    ₹${order.total || order.totalAmount || 0}*`);
  lines.push(`  Payment:   ${order.paymentMethod || "—"}`);
  lines.push(`  Status:    ✅ ${order.status || "Paid"}`);
...
/* ──────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────────
   Can be called standalone (e.g. from a button) for a specific
   order.  Looks up the order from global store and opens
...
function sendWhatsAppNotification(orderId) {
  ...
  const addr    = order.selectedAddress || {};
  const name    = addr.fullName    || order.userName || "Customer";
  const company = addr.companyName || order.companyName || "—";
  const address = addr.address     || order.address  || "—";
  const phone   = addr.phone       || order.phone    || "—";

  const waNumber = "91XXXXXXXXXX";   // ← Replace with your WhatsApp number
  const text     = _aoWhatsAppText(order, name, company, address, phone);
  window.open(`https://wa.me/${waNumber}?text=${encodeURIComponent(text)}`, "_blank");
}


/* ──────────────────────────────────────────────────────────────
   _aoGetWhatsAppURL(order)
   Helper that builds the full WhatsApp deep-link for a given
   order object — used in _opShowSuccessScreen template literal.
   ────────────────────────────────────────────────────────────── */
