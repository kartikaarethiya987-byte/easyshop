...
/* ============================================================
   PHASE 3 ADDITIONS – Authentication (localStorage-based)
   ============================================================ */

/* ----------------------------------------------------------
   AUTH STORAGE KEYS
   ---------------------------------------------------------- */
const USERS_KEY      = "shopeasy_users";        // Stores all registered users
const SESSION_KEY    = "shopeasy_session";       // Stores currently logged-in user


/* ----------------------------------------------------------
   GET ALL REGISTERED USERS
   Returns an array of { name, email, password } objects.
   ---------------------------------------------------------- */
...
/* ----------------------------------------------------------
   ...
   Returns { name, email } or null if not logged in.
   ---------------------------------------------------------- */
...
/* ----------------------------------------------------------
   REGISTER A NEW USER
   Checks for duplicate email, then saves to users array.
...
function registerUser(name, email, password) {
  ...
  // Save new user (NOTE: In real apps, NEVER store plain-text passwords!)
...
/* ----------------------------------------------------------
   ...
   If valid, saves a session object to localStorage.
   Returns { success: true, name: "..." } or { success: false, message: "..." }
   ---------------------------------------------------------- */
function loginUser(email, password) {
  ...
  // Save session (only store name + email, NOT password)
...
/* ----------------------------------------------------------
   LOGOUT USER
   Clears the session from localStorage and reloads navbar.
   ---------------------------------------------------------- */
function logoutUser() {
...
/* ----------------------------------------------------------
   RENDER NAVBAR AUTH AREA  (#navAuth)
   Called on every page to show the right state:
     - Not logged in → Login + Register buttons
     - Logged in     → "Hi, Name" chip + Logout button
   ---------------------------------------------------------- */
function renderNavAuth() {
  ...
  if (user) {
    // User is logged in — show greeting chip + logout button
    container.innerHTML = `
